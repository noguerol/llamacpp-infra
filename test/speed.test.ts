...
console.log("speed tracker: basic turn lifecycle");
{
	...
	check("prefill line before first token", lines.join() === "📊 · ⚡ prefill…", lines.join());

	// First token at t=2000 → streaming. No rate yet (window span < 300 ms).
	tracker.onToken(ctx, delta(1), 2000);
	check("streaming line shows tok count (rate pending)", /1 tok/.test(lines.join()), lines.join());
	...
	// Display lags ≤ 100 ms behind the last token (render throttle).
	const countM = line.match(/(\d+) tok/);
	const count = countM ? parseInt(countM[1], 10) : NaN;
	check("token count 28..30 (throttled display)", count >= 28 && count <= 30, line);
	...
	check("done line keeps gen rate + tok", /🔥 .*t\/s/.test(done) && /30 tok/.test(done), done);
	check("prefill rate 1500 shown", /⚡ 1500/.test(done), done);

	tracker.onTurnEnd(ctx);
	check("idle line after turn end", lines.join() === "📊 · ⏸ idle", lines.join());
	...
	check("server supplement shown while idle", /▶ 2/.test(sup) && /⚡ 150/.test(sup) && /🔥 18\.0/.test(sup) && /server/.test(sup), sup);

	tracker.onServerState(ctx, null);
	check("back to plain idle", lines.join() === "📊 · ⏸ idle", lines.join());
}
...
console.log("speed tracker: render throttle (no spam)");
{
	...
	check("final count 50", /50 tok/.test(lines.join()), lines.join());
...
console.log("speed tracker: dedup (no redundant status updates)");
{
	...
	tracker.onRequest(ctxDup, 0); // "📊 · ⚡ prefill…" → 1 update
